// 만세력 사주 앱 개발 기록 → docx 생성
import { Document, Packer, Paragraph, TextRun, HeadingLevel, AlignmentType,
  Table, TableRow, TableCell, WidthType, BorderStyle, ShadingType } from 'docx';
...
const P = (t, opts = {}) => new Paragraph({ spacing: { after: 90 }, ...opts,
  children: [new TextRun({ text: t, size: 22, color: INK })] });
const BULLET = (t, level = 0) => new Paragraph({ bullet: { level }, spacing: { after: 50 },
  children: [new TextRun({ text: t, size: 22, color: INK })] });
const CODE = (t) => new Paragraph({ spacing: { after: 90 }, shading: { type: ShadingType.SOLID, color: 'f0e9db' },
  children: [new TextRun({ text: t, font: 'Consolas', size: 20, color: '333333' })] });
const SPACER = () => new Paragraph({ spacing: { after: 60 }, children: [new TextRun('')] });

function table(headers, rows) {
  const border = { style: BorderStyle.SINGLE, size: 4, color: 'd8cbb2' };
  const borders = { top: border, bottom: border, left: border, right: border,
    insideHorizontal: border, insideVertical: border };
  const headerRow = new TableRow({ tableHeader: true, children: headers.map(h =>
    new TableCell({ shading: { type: ShadingType.SOLID, color: ACCENT },
      margins: { top: 60, bottom: 60, left: 90, right: 90 },
      children: [new Paragraph({ children: [new TextRun({ text: h, bold: true, color: 'ffffff', size: 20 })] })] })) });
  const bodyRows = rows.map((r, i) => new TableRow({ children: r.map(c =>
    new TableCell({ shading: { type: ShadingType.SOLID, color: i % 2 ? 'faf6ec' : 'ffffff' },
...
children.push(
  new Paragraph({ alignment: AlignmentType.CENTER, spacing: { before: 1400, after: 60 },
    children: [new TextRun({ text: '만세력 사주 조회 웹앱', bold: true, size: 56, color: ACCENT })] }),
...
children.push(
  ...
  H2('2.2 Node.js (포터블)'),
  P('이 PC에는 Node.js가 설치되어 있지 않았고, 관리자 권한(UAC) 문제로 MSI 설치가 불가하여 포터블 버전을 사용하였다.'),
  CODE('C:\\Users\\VIEW LIFE\\AppData\\Local\\nodejs-portable\\node-v24.19.0-win-x64'),
  P('빌드/배포 전 위 경로를 PATH에 주입해야 한다. 예:'),
  CODE('$env:Path="C:\\Users\\...\\nodejs-portable\\node-v24.19.0-win-x64;$env:Path"; node build_html.mjs'),
  H2('2.3 빌드 구조'),
...
children.push(
  ...
  BULLET('Firebase 프로젝트(manseayuk) 생성, 웹 앱 등록, Google 로그인 제공자 활성화, Firestore 데이터베이스 생성.'),
  BULLET('저장 구조: Firestore 문서 users/{uid} 의 profiles 필드에 사용자별 입력을 보관.'),
  BULLET('보안 규칙: 각 사용자는 자신의 users/{uid} 문서만 읽기/쓰기 가능.'),
  BULLET('프로필 저장 로직을 "메모리 캐시 + 백엔드 전환" 구조로 리팩터링 — 로그인 전에는 로컬, 로그인 후에는 Firestore.'),
  BULLET('최초 로그인 시, 브라우저에 저장돼 있던 입력을 계정으로 올릴지 확인하는 절차 제공.'),
  P('구글은 Firebase 기본 제공자라 별도 서버 없이 클라이언트 SDK만으로 로그인이 동작한다.')
);

// ── 4. 디자인 개편 ──
children.push(
  H1('4. 디자인 개편'),
  BULLET('상단 헤더를 로고+제목 구성 → 프리미엄 히어로(그라데이션) → 최종적으로 사용자가 제공한 대표 이미지 배너로 교체.'),
  BULLET('로그인 버튼을 이미지 위에 오버레이(어두운 반투명 알약), 로그인 후에는 밝은 칩 배경의 프로필로 전환.'),
  BULLET('저장·불러오기 카드는 로그인했을 때만 노출, 로그아웃 시 안내문으로 대체.'),
  BULLET('대표 이미지 우측 하단에 "萬歲曆 · 1900–2050 · 한국천문연구원(KASI) 기반" 캡션 오버레이.'),
  H2('4.1 대표 이미지 최적화'),
  P('대표 이미지가 2.3MB(PNG)로 초기 로딩이 느려, sharp 라이브러리로 WebP 변환·리사이즈하였다.'),
  table(['항목', '변환 전', '변환 후'],
    [['형식', 'PNG', 'WebP (품질 82)'],
     ['용량', '약 2,280 KB', '약 119 KB (약 95% 감소)'],
     ['해상도', '1536 × 1024', '1536 × 1024 (동일)']])
);

// ── 5. 네이버·카카오 로그인 ──
children.push(
  H1('5. 소셜 로그인 확장 (네이버·카카오)'),
  P('네이버와 카카오는 Firebase 기본 제공자가 아니므로, 정적 사이트만으로는 클라우드 저장과 연동할 수 없다. 이를 위해 Firebase Cloud Functions로 "커스텀 토큰 브리지"를 구축하였다.'),
  H2('5.1 동작 흐름'),
  BULLET('브라우저 → 네이버/카카오 인가(authorize) 페이지로 이동 (redirect_uri = Cloud Function 주소).'),
  BULLET('Cloud Function이 인가코드 → 접근토큰 교환(Client Secret 사용, 서버 전용).'),
  BULLET('접근토큰으로 프로필 조회(브라우저는 CORS로 불가, 서버에서 수행).'),
  BULLET('uid = "naver:<id>" 또는 "kakao:<id>" 로 Firebase 커스텀 토큰 발급.'),
  BULLET('사이트로 되돌리며 URL fragment(#)로 토큰 전달 → 브라우저가 signInWithCustomToken 으로 로그인.'),
  BULLET('결과적으로 기존 Firestore 규칙(users/{uid})과 그대로 호환되어 저장/불러오기가 동일하게 동작.'),
  H2('5.2 함수 및 비밀값'),
  BULLET('함수(1세대, 리전 asia-northeast3): naverCallback, kakaoCallback — URL이 예측 가능해 콜백 등록이 용이.'),
  BULLET('비밀값은 functions/.env(git 커밋 제외)에 보관: 네이버 Client ID/Secret, 카카오 REST API 키/Client Secret.'),
  BULLET('클라이언트에는 공개 가능한 값(네이버 Client ID, 카카오 REST 키)만 포함.'),
  H2('5.3 필수 IAM 권한'),
  P('커스텀 토큰은 서명을 위해 실행 서비스 계정에 "Service Account Token Creator" 역할이 필요하다. 없으면 signBlob 권한 거부 오류가 발생한다.'),
  CODE('계정: manseayuk@appspot.gserviceaccount.com  /  역할: roles/iam.serviceAccountTokenCreator'),
  H2('5.4 카카오 연동 중 발생한 오류와 해결 (KOE)'),
  table(['오류', '의미', '해결'],
    [['KOE004', '앱 관리자 설정 오류(동의항목)', '이메일 등 비즈앱 전용 항목을 필수 동의에서 해제, 닉네임만 필수로'],
     ['KOE006', 'Redirect URI 불일치', '카카오 로그인의 Redirect URI 칸에 함수 URL을 정확히 등록(사이트 도메인 아님)'],
     ['KOE010', 'Bad client credentials', 'Client Secret "사용함" 상태에서 미전송 → .env에 KAKAO_CLIENT_SECRET 설정 후 재배포']])
);
...
children.push(
  ...
  BULLET('admin.html: 관리자 구글 로그인 → 통계(전체/구글/네이버/카카오/저장보유) + 회원 표 + 저장 프로필 펼쳐보기.'),
  BULLET('검색 노출 차단(noindex)이며 어디에도 링크하지 않음. 비관리자는 서버에서 데이터가 차단되어 URL 노출돼도 안전.'),
  BULLET('메인 사이트 헤더에는 관리자 계정으로 로그인했을 때만 "관리자" 링크가 나타남.'),
...
children.push(
  ...
  table(['구성요소', '위치 / 값'],
    [['정적 사이트', 'GitHub Pages (main 브랜치, index.html)'],
     ...
     ['소셜 로그인', '구글(네이티브) · 네이버 · 카카오(커스텀 토큰 브리지)']]),
  SPACER(),
  P('※ 보안을 위해 이 문서에는 실제 비밀키(네이버/카카오 Client Secret 등)를 기재하지 않았다. 해당 값은 functions/.env 에만 저장되며 저장소에는 커밋되지 않는다.', { children: undefined })
);
